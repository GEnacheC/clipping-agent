@echo off
REM build.bat — Gera o executável clipping_agent.exe
REM Uso: execute este arquivo na raiz do projeto

echo [1/3] Instalando dependencias...
pip install -r requirements.txt pyinstaller --quiet
if errorlevel 1 (
    echo ERRO: falha ao instalar dependencias
    exit /b 1
)

echo [2/3] Baixando dados do NLTK...
python -c "import nltk; nltk.download('punkt', quiet=True); nltk.download('punkt_tab', quiet=True); nltk.download('stopwords', quiet=True)"
if errorlevel 1 (
    echo ERRO: falha ao baixar dados do NLTK
    exit /b 1
)

echo [3/3] Compilando executavel...
pyinstaller clipping_agent.spec --noconfirm
if errorlevel 1 (
    echo ERRO: falha na compilacao
    exit /b 1
)

echo.
echo ============================================================
echo Executavel gerado em: dist\clipping_agent.exe
echo Copie o arquivo .env para a mesma pasta do executavel.
echo ============================================================
